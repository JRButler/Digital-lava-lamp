...
//colour temperature
float cmin = -100; //dark dark purple; default -100
float clow = -40; //dark purple; default -40
float chigh = 40; //deep red; default 40
float chot = 60; //orange; default 60
float cmax = 120; //mid orange; default 120
float copq = 255; // yellow fill opacity 0-255; default 100
...
void draw(){
  ...
  for ( I = 0 ; I <= Nmax ; I++ ){
    ...
    //{ stroke(255,255*(T[I]-chigh)/(chot-chigh),0,0) ; 
    //fill(255,255*(T[I]-chigh)/(chot-chigh),0,copq) ; }

    if ( T[I] <= chot && T[I] >= chigh )
    { stroke(255,0,55-255*(T[I]-chigh)/(chot-chigh),0) ; 
     fill(255,0,55-255*(T[I]-chigh)/(chot-chigh),copq) ; }
    
    if ( T[I] < chigh && T[I] >= clow )
    { stroke(255*(T[I]-clow)/(chigh-clow),0,35,0) ; 
     fill(255*(T[I]-clow)/(chigh-clow),0,35,copq) ; }
    
    if ( T[I] < clow && T[I] >= cmin )
    { stroke((T[I]-cmin)/(clow-cmin),0,10,0) ; 
     fill((T[I]-cmin)/(clow-cmin),0,10,copq) ; }
    
    if ( T[I] < cmin )
    { stroke(33,0,0,0) ; fill(33,0,0,copq) ; }
